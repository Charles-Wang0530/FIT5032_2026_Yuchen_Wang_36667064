...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <p>We operate in: {{ bookstores.locations.join(", ") }}</p>
    ...
    <section class="lab-section">
      ...
      <ul>
        <li
          ...
          :key="author.id"
          :class="{ highlight: highlightedAuthorId === author.id }"
          :style="{
            fontWeight: highlightedAuthorId === author.id ? 'bold' : 'normal',
            color: highlightedAuthorId === author.id ? '#ffffff' : '#2c3e50'
          }"
          :title="`Click to highlight ${author.name}`"
